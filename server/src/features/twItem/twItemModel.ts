...
import { ItemType, ItemSetDetailType } from './twItemType'

// TwItem
// @doc the basic stock unit.

// @todo Maybe this model can remember last update user_id
const TwItemSchema = new mongoose.Schema<ItemType.Mongoose>(
	{
		owner: {
...
const TwItemSetDetailSchema =
	new mongoose.Schema<ItemSetDetailType.TwItemSetDetailType>({
		owner: {
			type: Schema.Types.ObjectId,
			ref: 'user',
			required: true,
